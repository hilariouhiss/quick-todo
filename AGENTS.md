...
- 项目通过「＋ 添加项目」**弹窗**创建（名称必填），可带可选**起止时间**（侧边栏小字展示，内联编辑可改）；**任务弹窗内可快速新建项目**（仅名称，创建后自动选中）
